# 个性化五音养生音乐生成与评估系统 · 学术实验网站（v4.0 双端科研版）
...
- **对照三型**（`CONFIG.controlType`，默认粉红噪声）：
  - `pinknoise`（默认）：Web Audio 离线合成柔和粉红噪声（Voss-McCartney 近似 + 低通 + 淡变），不增加素材、人人一致；
...
- **客观指标只采 HR + HRV(RMSSD)**（导师拍板，消费级 Apple Watch 无 EDA，血氧/血压不进被试端）：
  听前静坐基线、听后各录一次，页面内置 Apple Watch 标准测量指引（静坐 3–5 min、同一 App、HRV 测满 1 min 取 RMSSD）；
- **主观量表**：SAM 效价/唤醒/支配（自绘图形规避官方版权）+ STAI-S 20 题（自动反向计分），听前后各一套；
